...
def moving_right_in_oblast(ramka_obj, unit_object,):
    ...
    if future_node in unit_object.link_to_path.get_allowed_obl():
        ramka_obj.move_right = True
        ramka_obj.update()
        peres4et_puti(ramka_obj, unit_object,)


def moving_left_in_oblast(ramka_obj, unit_object,):
...
def moving_up_in_oblast(ramka_obj, unit_object,):
...
def moving_down_in_oblast(ramka_obj, unit_object,):
    future_node = ramka_obj.get_koordinate()[0] + 1, ramka_obj.get_koordinate()[1]
